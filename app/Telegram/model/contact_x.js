	...
	if (phoneCrack) {
		Phone.findOne({ 'phone': phoneCrack.phone }, 'phone', function (err, check1) {
			if (check1) {
				try {
					telegram.create({ 'form': id, 'phone': phoneCrack.phone }, function (err, cP) {
						if (!!cP) {
							...
							bot.sendMessage(id, '_Failed action_', { parse_mode: 'markdown', reply_markup: { remove_keyboard: true } });
						}
					});
				} catch (error) {
					bot.sendMessage(id, '_Failed action_', { parse_mode: 'markdown', reply_markup: { remove_keyboard: true } });
				...
				bot.sendMessage(id, 'Số điện thoại này chưa được đăng ký. Vui lòng đăng ký tại _KING86.CLUB_', { parse_mode: 'markdown', reply_markup: { remove_keyboard: true } });
			}
		});
	}
}
